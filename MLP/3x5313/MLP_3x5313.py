...
def load_data_from_h5(directory):
    ...
    test_Ts = []
    test_Ts = [800,859,900,977,1000,1100, 1152, 1200]
    ...
    # for i,j in zip(T_values_fin, real_imag_data_fin):
    #    T_values.append(i )# + np.random.normal(loc=0, scale=0.001, size=1))
    #    real_imag_data.append(j + np.abs(j)*np.random.normal(loc=0, scale=0.001, size=1))
...
def main():
    ...
    # 1. Load data
    # -----------------------
    data_directory = '/Volumes/T7 Shield/T_full_data/3x5313_spectrograms'
    ...
    T_norm = scaler_T.fit_transform(T**3)
    # T_norm = np.array(T**2)
    ...
    for new_T in test_Ts:
        ...
        # -----------------------
        output_filename = f"../../ML_prediction/3x5313/predicted_spectrogram_{new_T}_real_imag.h5"
