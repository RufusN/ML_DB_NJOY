...
spectrogram_dir = Path('/Users/ru/FFT/ML_DB_NJOY/ML_prediction_U238')
# Path to XS HDF5 data files (one per temperature)
xs_data_dir = Path('/Volumes/T7 Shield/NJOY/U238_data')
...
# ---------------------------
base_file = xs_data_dir / 'U238_300.0.h5'
Base_Tval = 300.0
try:
    ...
    df_base = pd.DataFrame(values, columns=items)
except Exception as e:
    print(f"Error reading base XS file: {e}")
    exit(1)
...
# Apply energy mask
mask = (df_base['ERG'] * 1e-6 >= E_min) & (df_base['ERG'] * 1e-6 <= E_max)
subset_base = df_base[mask].copy()
# Convert ERG to MeV
subset_base['ERG'] = subset_base['ERG'] * 1e-6

Base_E = subset_base['ERG'].to_numpy()
...
for spec_file in spectrogram_files:
    ...
    # ---------------------------
    xs_file = xs_data_dir / f'U238_{T_val}.0.h5'
    try:
