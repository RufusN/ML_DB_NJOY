#!/usr/bin/env python3
"""table_xs.py – fast Doppler‑XS lookup & GPU benchmark (latent‑major)
--------------------------------------------------------------------
* Latent‑major **W_tab** (16×E) – no transpose in matmul.
* Optional **float‑16 table** via `--dtype float16` (keeps fp32 accum).
* **XLA** opt‑in with `--xla` (CPU only).
* Accurate timing; auto‑prints ns/value when very small.
"""
...
# ═════════════ single query (fp32 accum) ═════════════════════════════════

@tf.function
def _tf_query(T_batch, E_batch, W_tab, b_tab, W0, b0, alpha, T_scale, T_mean):
    T_norm   = (T_batch - T_mean) / T_scale
    hidden16 = tf.nn.leaky_relu(tf.matmul(T_norm[:,None], W0) + b0, alpha)   # fp16/32
    W_sel16  = tf.gather(W_tab, E_batch, axis=1)
    b_sel16  = tf.gather(b_tab, E_batch)

    hidden32 = tf.cast(hidden16, tf.float32)
    xs32 = tf.matmul(hidden32, tf.cast(W_sel16, tf.float32)) + tf.cast(b_sel16, tf.float32)
    return xs32  # fp32 result for robustness
...
def query_xs(table_h5: str, T: float, E_idx: int):
    ...
    val = _tf_query(tf.constant([T]), tf.constant([E_idx], tf.int32),
                    W_tab, b_tab, W0, b0, alpha, T_scale, T_mean)
    return float(val.numpy()[0])

# ═════════════ batch benchmark (fp32 accum) ═════════════════════════════=
...
def batch_query(table_h5: str, T_arr, E_arr, use_gpu: bool, xla: bool):
    ...
    with tf.device(device):
        ...
        hidden16 = tf.nn.leaky_relu(tf.matmul(((T_tf - T_mean)/T_scale)[:,None], W0) + b0, alpha)
        W_sel16  = tf.gather(W_tab, tf.constant(E_arr, tf.int32), axis=1)
        b_sel16  = tf.gather(b_tab, tf.constant(E_arr, tf.int32))

        hidden32 = tf.cast(hidden16, tf.float32)
        W_sel32  = tf.cast(W_sel16, tf.float32)
        b_sel32  = tf.cast(b_sel16, tf.float32)

    start = time.perf_counter()
    with tf.device(device):
        xs = tf.matmul(hidden32, W_sel32) + b_sel32   # fp32 accumulate
        xs_np = xs.numpy()
...
def main():
    ...
    p_batch.add_argument("--xla", action="store_true", help="enable XLA JIT (CPU only – Metal GPU jit unsupported)")
