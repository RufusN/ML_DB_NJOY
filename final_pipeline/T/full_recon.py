...
#window_size = 0.010285
#step_size =   0.003471
#E_min = 1e4 * 1e-6 #MeV
#E_max = 1e5 * 1e-6 #MeV


##### forget the ones above here ####################
...
path = '/mnt/c/Users/marti/Documents/Martin/SCONE/code_stuff/Scripts/DB_sliced/ML_DB_sliced_pipeline/final_pipeline/T/ML_predictions/'
...
for file in h5_files:
    ...
    with h5py.File(file, "r") as h5f:
        ...
        # Prepare for inverse overlap-add
        # padded_signal_length = step_samps * (spectrogram.shape[1] - 1) + window_samps
        padded_signal_length = 2 * (window_samps) + sampling_size
        reconstructed_signal = np.zeros(int(padded_signal_length), dtype=np.float64)
        overlap_factor = np.zeros(int(padded_signal_length), dtype=np.float64)
        ...
        # reconstructed_signal = np.zeros((len(time_bins) - 1) * step_samps + window_samps)
        ...
        for file_path in all_files:
            ...
            plt.tight_layout()
            plt.savefig(f'./results_full_recon/xs_comp_{T_val}.png', dpi=200)
            plt.close()
            ...
            # Second plot: Relative Error
            axs[1].plot(E, np.array(relative_error)*100, label='Relative Error', color='red', alpha=0.8)
            axs[1].set_xlabel("Energy (eV)")
            axs[1].set_ylabel("Relative Error %")
            axs[1].set_title(f"Relative Error (T={T_val})\nE in [{E_min:.2f}, {E_max:.2f}]")
            axs[1].legend()
            ...
            plt.tight_layout()
            plt.savefig(f'./results_full_recon/xs_comp_linear_{T_val}.png', dpi=200)
            plt.close()
            break
...
plt.savefig(f'./results_full_recon/performance.png', dpi=200)
plt.close()
...
plt.legend(fontsize=12)
plt.tight_layout()
plt.savefig(f'./results_full_recon/RelErrDistr.png', dpi=200)
plt.close()
...
plt.axhline(target_relative_error, color='red', linestyle='--', linewidth=1.5, label='Target Relative Error')
plt.legend(fontsize=12)
plt.savefig(f'./results_full_recon/RelErrDistrT.png', dpi=200)
plt.close()