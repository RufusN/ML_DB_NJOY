...
def analyse(pad, padded_Base_E, padded_sig, E_indices, results):
    ...
    plt.yscale("log")
    #plt.show()
    fname = "./results_point_recon/xs.png"
    ...
    plt.legend()
    #plt.show()
    fname = "./results_point_recon/relative_error.png"
    plt.savefig(fname, dpi=200)
    plt.close()
    return None
...
def main():
    with h5py.File("/mnt/c/Users/marti/Documents/Martin/SCONE/code_stuff/Scripts/DB_sliced/ML_DB_sliced_pipeline/final_pipeline/T/3x45551_950_1050_spec_scalers.h5", "r") as hf:
    ...
    padded_Base_E, padded_sig = load_temperature(test_temp, Base_E, pad, E_min, E_max, file_path=r'/mnt/d/800_1200')

    E_indices = [i for i in range(pad,len(Base_E)-pad)]
    results = []
    model = load_model('/mnt/c/Users/marti/Documents/Martin/SCONE/code_stuff/Scripts/DB_sliced/ML_DB_sliced_pipeline/final_pipeline/T/3x45551_950_1050.h5', compile = False)
    for E_idx in E_indices:

        print(str(E_idx) + '/' + str(len(range(pad,len(Base_E)-pad))))
        ...
        spectrogram = get_spectrogram_output(model, input_data, scaler_T, scaler_spec, seg_indices, h, w, c)
