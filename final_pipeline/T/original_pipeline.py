...
MODEL_PATH = '/mnt/c/Users/marti/Documents/Martin/SCONE/code_stuff/Scripts/DB_sliced/ML_DB_sliced_pipeline/3x45551_point_recon/martin_pipeline/3x45551_950_1050.h5'
...
def analyse(base_e, reconstructed, temps, eidxs, file_dir):
    ...
    plt.savefig('./results_original_inference/xs.png', dpi=200)
    plt.close()
    ...
    plt.xscale('log'); plt.yscale('log'); plt.xlabel('Energy Index'); plt.ylabel('Relative Error (%)')
    plt.title('Relative Error vs Energy'); plt.grid(True)
    plt.savefig('./results_original_inference/relative_error.png', dpi=200)
    plt.close()
    ...
    plt.figure(figsize=(8,5))
    plt.plot(base_e[idxs_sorted-PAD], rel_err_sorted, marker='o', linestyle='-')
    plt.xlabel('Energy Index'); plt.ylabel('Relative Error (%)')
    plt.title('Relative Error vs Energy'); plt.grid(True)
    plt.savefig('./results_original_inference/relative_error_linear.png', dpi=200)
    plt.close()
